...
apiRoute.all(async (request: NextApiRequest, response: NextApiResponse, next: NextHandler) => {
...
apiRoute.put(async (request: NextApiRequest, response: NextApiResponse) => {
    try {
        ...
        response.status(500).json({
            error: error?.message || 'Une erreur est survenue lors de la modification du guide',
        });
    }
});
...
apiRoute.delete(async (request: NextApiRequest, response: NextApiResponse) => {
    try {
        ...
        console.error(error);
        response.status(500).json({
            error: error?.message || 'Une erreur est survenue lors de la suppression du guide',
        });
    }
});
